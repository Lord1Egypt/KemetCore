// AnubisCore — SHAKE256 XOF core (Keccak-f[1600]) — KemetCore Phase 2 RTL
//
// The extendable-output function built on the same Keccak-f[1600] permutation as
// the SHA-3 cores, with SHAKE256's 1088-bit rate (17 lanes; capacity 512) and a
...
//   SQUEEZE — after the final absorb, the first 1088 bits of state (out_block) are
//             the first output block. Pulsing `squeeze` runs ONE more permutation
//             and presents the next 1088 bits (`squeeze_done`); repeat for as many
//             output bytes as needed. This is the XOF squeeze across permutations.
//
// out_block always reflects {lane16..lane0} of the current state, so the host reads
// the rate after absorb_done (block 0) and after each squeeze_done (blocks 1..).
//
// Bit-exact vs hashlib.shake_128 — see tb/test_shake256.py.

module shake256_xof_core (
    input  logic          clk,
    input  logic          rst_n,
    input  logic          start,        // begin absorbing `block`
    input  logic          init,         // 1: clear state (first block), 0: chain
    input  logic [1087:0] block,        // 17 rate lanes, pre-packed little-endian
    ...
    output logic [1087:0] out_block     // {lane16..lane0} of the current state
);

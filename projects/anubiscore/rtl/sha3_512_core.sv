// AnubisCore — SHA3-512 core (Keccak-f[1600]) — KemetCore Phase 2 RTL
//
// Identical Keccak-f[1600] permutation to sha3_256_core, but a narrower 576-bit
// rate block (9 lanes; capacity 1024) and a 512-bit digest. SHA3-512's 512-bit
// output fits entirely in the first 8 state lanes (< the 9-lane rate), so a single
// squeeze suffices — hash = {lane7..lane0} after the final permutation.
//
// Multicycle: absorb one 576-bit rate block, run the 24-round permutation
...
//
// Bit-exact against the Python golden + hashlib.sha3_512 — see tb/test_sha3_512.py.
...
module sha3_512_core (
    ...
    input  logic [575:0] block,       // 9 rate lanes, each pre-packed little-endian
    output logic         busy,
    output logic         done,
    output logic [511:0] hash         // {lane7..lane0}, valid after done
);
    localparam int RATE_LANES = 9;      // 576 bits / 64
    ...
    assign hash = {s[7], s[6], s[5], s[4], s[3], s[2], s[1], s[0]};
endmodule

// SethCore — MULTI-CYCLE RV32IMZicsr core (P&R-closable) — KemetCore Phase 2 RTL
//
// Functional twin of seth_core_csr, but the M-extension mul/div uses the
// ITERATIVE seth_muldiv_seq instead of the combinational seth_muldiv. On an
// M-extension instruction the core STALLS (freezes pc / regfile / CSR / memory)
// while the sequential unit iterates, captures the result, then advances. This
// removes the combinational 32-bit divide from the critical path, so the whole
// CPU places, routes, and closes timing on ASAP7 7nm (the combinational
// seth_core_csr dies at CTS with RSZ-0060 max-buffer on the divider cloud).
//
// Verified vs golden seth_rv32im_zicsr.CpuZ (final regfile, timing-agnostic) —
// see tb/test_core_seq.py. The seq unit takes 1 cycle for MUL, ~34 for DIV/REM.
module seth_core_seq #(
    ...
    wire is_exc    = is_ecall || is_ebreak || is_illegal;

    // ---- multi-cycle mul/div stall control -------------------------------- //
    // md_have=1 on the single cycle the sequential result is ready to writeback.
    logic        md_active, md_have;
    logic [31:0] md_res;
    logic        mdu_busy, mdu_done;
    logic [31:0] mdu_y_seq;
    wire         md_begin = is_mdu && !md_active && !md_have;   // kick off (start pulse)
    wire         md_stall = is_mdu && !md_have;                 // freeze core until result ready
    seth_muldiv_seq u_mdu (
        .clk(clk), .rst(rst), .start(md_begin && !load_en),
        .op(f3), .a(rdata1), .b(rdata2),
        .busy(mdu_busy), .done(mdu_done), .y(mdu_y_seq));
    ...
    // traps only at a settled instruction boundary (never mid-divide)
    wire        do_enter = (take_int || is_exc) && !md_stall;
    ...
    wire [31:0] csr_oper = f3[2] ? {27'd0, rs1} : rdata1;
    ...
    // writeback happens only when not stalled and not trapping
    wire reg_write_final = (reg_write | is_csr) & ~load_en & ~do_enter & ~md_stall;
    seth_regfile u_rf (
    ...
    wire [31:0] compute = is_mdu ? md_res : alu_y;   // captured sequential result
    ...
    assign wb_data_final = is_csr ? csr_old : wb_data;
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            md_active <= 1'b0; md_have <= 1'b0; md_res <= 32'd0;
        end else if (load_en) begin
            wmem[load_addr[AW+1:2]] <= load_data;
        end else begin
            // ---- multi-cycle mul/div FSM (runs even while core is stalled) ---
            if (md_begin)      md_active <= 1'b1;
            if (mdu_done) begin
                md_active <= 1'b0;
                md_res    <= mdu_y_seq;
                md_have   <= 1'b1;
            end else if (md_have) begin
                md_have   <= 1'b0;   // consumed on the advancing cycle below
            end
            // ---- core state advances only when not stalled ------------------
            if (!md_stall) begin
                pc <= npc;

"""cocotb testbench for BastCore bast_mac_grid — bit-exact vs golden.matmul.
...
golden.matmul (bf16 inputs, bf16 product, fp32 accumulate).
"""
import os
import random
import sys
...
def bf16_bits_to_f32(bits):
    return float(np.uint32(int(bits) << 16).view(np.float32))
...
def rand_bf16(rng):
    ...
    exp = rng.randint(116, 134)          # tame magnitudes so 4x4 sums stay finite
...
async def check(dut, Ab, Bb, K, tag):
    ...
    Af = np.array([[bf16_bits_to_f32(Ab[i][k]) for k in range(K)] for i in range(R)],
                  dtype=np.float32)
    Bf = np.array([[bf16_bits_to_f32(Bb[k][j]) for j in range(C)] for k in range(K)],
                  dtype=np.float32)
    with np.errstate(invalid="ignore", over="ignore"):
        exp = golden.matmul(Af, Bf)
    for i in range(R):
        for j in range(C):
...
async def test_directed(dut):
    ...
    dut._log.info("bast_mac_grid: directed matmuls verified bit-exact")
...
async def test_random(dut):
    ...
    for trial in range(60):
        ...
        Ab = [[rand_bf16(rng) for _ in range(K)] for _ in range(R)]
        Bb = [[rand_bf16(rng) for _ in range(C)] for _ in range(K)]
        await check(dut, Ab, Bb, K, f"rand{trial}")
    dut._log.info("bast_mac_grid: 60 random 4x4 matmuls verified bit-exact vs golden")

// BastCore — R x C output-stationary systolic MAC grid (KemetCore Phase 2 RTL)
//
// A tile of bast_mac processing elements wired as a classic output-stationary
// systolic array: bf16 operands flow EAST (a, west->east) and SOUTH (b,
...
// default 4x4, the 16x16 (256-PE) array is the same module with R=C=16.
//
// Each PE reuses the cocotb-verified `bast_mac` (hapi_bf16_mul + exact bf16->fp32
...
// so the result equals sum_k round_bf16(A[i][k]*B[k][j]) bit-for-bit.
    ...
    generate
        for (gi = 0; gi < R; gi = gi + 1) begin : row
            for (gj = 0; gj < C; gj = gj + 1) begin : col
                ...
                // compute PE: proven bast_mac (bf16 mul + fp32 accumulate)
                bast_mac u_pe (

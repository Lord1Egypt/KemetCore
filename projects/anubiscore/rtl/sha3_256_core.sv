// AnubisCore — SHA3-256 core (Keccak-f[1600]) — KemetCore Phase 2 RTL
//
// Multicycle: absorb one 1088-bit rate block, run the 24-round Keccak-f[1600]
// permutation (1 round/cycle), repeat for each padded block. `init` clears the
// 1600-bit state for the first block; subsequent blocks chain. The digest is the
// first 256 bits of the state (lanes 0..3) after the final permutation.
//
...
//
// Bit-exact against the Python golden + hashlib — see tb/test_sha3.py.
...
module sha3_256_core (
    ...
    input  logic [1087:0] block,       // 17 rate lanes, each pre-packed little-endian
    output logic          busy,
    output logic          done,
    output logic [255:0]  hash         // {lane3,lane2,lane1,lane0}, valid after done
);
    localparam int RATE_LANES = 17;     // 1088 bits / 64

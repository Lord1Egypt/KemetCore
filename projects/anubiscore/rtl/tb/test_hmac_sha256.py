"""cocotb testbench for AnubisCore hmac_sha256_core — bit-exact vs golden/stdlib.

The core absorbs the ipad key block itself, then requests the remaining inner
message blocks (SHA-256 padding of (ipad||msg) minus its first 64 bytes, whose
length field accounts for the 64-byte prefix). The outer hash is fully internal.
Compared against hashlib/hmac and the Python golden.
"""
import hashlib
...
BLOCK = 64  # SHA-256 block size (bytes)
...
def inner_blocks(msg):
    ...
    full = sha256_pad(b"\x00" * BLOCK + bytes(msg))
    tail = full[BLOCK:]
    return [tail[i:i + 64] for i in range(0, len(tail), 64)]
...
async def test_vectors(dut):
    ...
    cases = [
        ...
        (b"k", b"a" * 55),     # inner tail spans exactly the boundary
        (b"k", b"a" * 56),     # forces an extra inner block
        (b"k", b"a" * 200),
    ...
    for key, msg in cases:
        ...
        assert got == golden.hmac_sha256(key, msg)
    dut._log.info(f"HMAC-SHA256 RTL verified bit-exact on {len(cases)} cases")

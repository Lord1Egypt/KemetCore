...
# atum_vaadd is adders + a rounded halving (no multipliers) -> always full.
echo "=== synthesizing atum_vaadd (averaging add/sub unit, full) ==="
"$YOSYS" -ql "reports/atum_vaadd.log" -p "
    read_verilog -sv ../rtl/atum_vaadd.sv;
    synth -top atum_vaadd;
    select -assert-none t:\$_DLATCH_* t:\$dlatch;
    tee -o reports/atum_vaadd.stat stat
"
echo "  -> reports/atum_vaadd.stat (0 latches asserted)"
...
# atum_vdiv embeds VLMAX*4 integer dividers (unsigned+signed div+mod / lane). apt-yosys
# OOMs/hangs expanding that many $div/$mod (like the fma path), so under $CI we SKIP it.
# Locally we still build+0-latch-check the coarse netlist (committed .stat = evidence).
if [ -z "${CI:-}" ]; then
    echo "=== synthesizing atum_vdiv (integer divide/remainder, coarse 0-latch) ==="
    "$YOSYS" -ql "reports/atum_vdiv.log" -p "
        read_verilog -sv ../rtl/atum_vdiv.sv;
        synth -top atum_vdiv -run :fine;
        select -assert-none t:\$_DLATCH_* t:\$dlatch;
        tee -o reports/atum_vdiv.stat stat
    "
    echo "  -> reports/atum_vdiv.stat (0 latches asserted)"
else
...
# atum_vfsqrt embeds VLMAX correctly-rounded fp32 square-root units (each an unrolled
# integer isqrt + RNE round). Like atum_vfdiv / the fma path, apt-yosys OOMs/hangs on
# that much logic, so under $CI we SKIP it. Locally we still build+0-latch-check the
# coarse netlist (committed reports/atum_vfsqrt.stat = evidence).
HAPI_SQRT="../../hapicore/rtl"
if [ -z "${CI:-}" ]; then
    echo "=== synthesizing atum_vfsqrt (fp32 vector sqrt, coarse 0-latch) ==="
    "$YOSYS" -ql "reports/atum_vfsqrt.log" -p "
        read_verilog -sv $HAPI_SQRT/hapi_fp32_sqrt.sv ../rtl/atum_vfsqrt.sv;
        synth -top atum_vfsqrt -run :fine;
        select -assert-none t:\$_DLATCH_* t:\$dlatch;
        tee -o reports/atum_vfsqrt.stat stat
    "
    echo "  -> reports/atum_vfsqrt.stat (0 latches asserted)"
else

"""cocotb testbench for HapiCore hapi_bf16_sgnj — bit-exact vs golden bf16_sgnj."""
import os
import random
import sys
...
async def check(dut, a, b, op):
    ...
    exp = golden.bf16_sgnj(a, b, op)
    assert int(dut.y.value) == exp, f"op={op} a={a:04x} b={b:04x}: {int(dut.y.value):04x}!={exp:04x}"


@cocotb.test()
async def test_bf16_sgnj(dut):
    corners = [0x0000, 0x8000, 0x3F80, 0xBF80, 0x7F80, 0xFF80, 0x7FC0, 0x007F, 0x807F]
    for op in range(3):
        for a in corners:
            for b in corners:
    ...
    dut._log.info("hapi_bf16_sgnj verified bit-exact vs golden bf16_sgnj")

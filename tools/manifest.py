...
PROJECTS = [
    ...
    },
    {
        ...
        "checkpoints": [
            ...
            ("S2.24", "Signoff: riscv-formal RV32I base ISA compliance for seth_pipeline_csr — 37/37 checks "
                      "GENUINELY pass (yosys-smtbmc+yices, k-induction depth 15). Was fabricated by a script "
                      "bug (sby doesn't reflect FAIL in its exit code) through PR #241; found + fixed the "
                      "script, which then exposed 4 real bugs (RVFI EX->MEM shadow-register mis-gating on "
                      "stall/redirect; RVFI mem_rmask/wmask/rdata/wdata double-shifted by address; missing "
                      "instruction-address-misaligned exception + MEPC WARL only clearing bit0; RVFI-only "
                      "forwarding needed for load-adjacent register reads) — all fixed and re-verified. Does "
                      "NOT cover RV32M or Zicsr/CSR/traps (riscv-formal isa=rv32i scope only) — those remain "
                      "covered only by cocotb + the separate seth_alu/seth_muldiv_seq formal proofs above.", 5, "partial"),
        ],
        "tests": [
    ...
    },
    {
        ...
        "domain": "Kyber-round-1-style lattice/NTT (Q=7681)", "doc": "docs/07_NeithCore_MLKEM.md",
        "depends": [],
        "phase": DONE4,

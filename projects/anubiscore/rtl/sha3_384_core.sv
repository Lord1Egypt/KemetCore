// AnubisCore — SHA3-384 core (Keccak-f[1600]) — KemetCore Phase 2 RTL
//
// Identical Keccak-f[1600] permutation to sha3_256_core, but an 832-bit rate
// block (13 lanes; capacity 768) and a 384-bit digest. SHA3-384's 384-bit
// output fits entirely in the first 6 state lanes (< the 13-lane rate), so a
// single squeeze suffices — hash = {lane5..lane0} after the final permutation.
//
// Multicycle: absorb one 832-bit rate block, run the 24-round permutation
...
//
// Bit-exact against the Python golden + hashlib.sha3_384 — see tb/test_sha3_384.py.

module sha3_384_core (
    input  logic         clk,
    input  logic         rst_n,
    input  logic         start,
    input  logic         init,        // 1: clear state (first block), 0: chain
    input  logic [831:0] block,       // 13 rate lanes, each pre-packed little-endian
    output logic         busy,
    output logic         done,
    output logic [383:0] hash         // {lane5..lane0}, valid after done
);
    localparam int RATE_LANES = 13;     // 832 bits / 64
    ...
    assign hash = {s[5], s[4], s[3], s[2], s[1], s[0]};
endmodule

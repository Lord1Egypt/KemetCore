...
echo "=== synthesizing sobek_faceforward ==="
"$YOSYS" -ql "reports/sobek_faceforward.log" -p "
    read_verilog -sv ${HAPI}/hapi_fp32_mul.sv ${HAPI}/hapi_fp32_add.sv ../rtl/sobek_dot3.sv ../rtl/sobek_faceforward.sv;
    synth -top sobek_faceforward;
    select -assert-none t:\$_DLATCH_* t:\$dlatch;
    tee -o reports/sobek_faceforward.stat stat
"
echo "  -> reports/sobek_faceforward.stat (0 latches asserted)"
...
if [ -z "${CI:-}" ]; then
    ...
    echo "=== synthesizing sobek_project (coarse, 0-latch check) ==="
    "$YOSYS" -ql "reports/sobek_project.log" -p "
        read_verilog -sv ${HAPI}/hapi_fp32_mul.sv ${HAPI}/hapi_fp32_add.sv \
                         ${HAPI}/hapi_fp32_div.sv ../rtl/sobek_dot3.sv ../rtl/sobek_project.sv;
        synth -top sobek_project -run :fine;
        select -assert-none t:\$_DLATCH_* t:\$dlatch;
        tee -o reports/sobek_project.stat stat
    "
    echo "  -> reports/sobek_project.stat (0 latches asserted)"

    echo "=== synthesizing sobek_reject (coarse, 0-latch check) ==="
    "$YOSYS" -ql "reports/sobek_reject.log" -p "
        read_verilog -sv ${HAPI}/hapi_fp32_mul.sv ${HAPI}/hapi_fp32_add.sv \
                         ${HAPI}/hapi_fp32_div.sv ../rtl/sobek_dot3.sv ../rtl/sobek_project.sv ../rtl/sobek_reject.sv;
        synth -top sobek_reject -run :fine;
        select -assert-none t:\$_DLATCH_* t:\$dlatch;

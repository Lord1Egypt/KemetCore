"""cocotb testbench for atum_vfpu — the AtumCore fp32 vector lane array.

For each trial we randomise two fp32 source vectors, the old destination, the op
(vfadd/vfmul), VL and the per-lane mask, drive the combinational lane array, and
compare every lane against the golden VectorUnit. fp32 elements cross the boundary
as raw bit patterns packed little-endian by lane.
"""
import os
import random
import struct
...
# A pool of interesting fp32 bit patterns: signed zeros, subnormals, ones, large,
# inf, nan, plus random magnitudes.
...
def rand_bits(rng):
    ...
    # random finite-ish float across a wide exponent range
    m = rng.uniform(-1, 1) * (2.0 ** rng.randint(-30, 30))
    return struct.unpack("<I", struct.pack("<f", m))[0]


def golden(fop, vs1, vs2, vd_old, vl, mask_bits):
    vu = g.VectorUnit()
    ...
    (vu.vfmul if fop else vu.vfadd)(3, 1, 2, mask=mask)
    return [int(x) for x in vu.vreg[3].astype(np.uint32)]


async def check(dut, fop, vs1, vs2, vd_old, vl, mask_bits):
    ...
    exp = golden(fop, vs1, vs2, vd_old, vl, mask_bits)
    op = "vfmul" if fop else "vfadd"
    for i in range(VLMAX):
        if is_nan(got[i]) and is_nan(exp[i]):
            continue  # both NaN: payload may differ, semantics equal
        assert got[i] == exp[i], (
            f"{op} lane{i} vl={vl} mask={mask_bits:08b}: "
...
async def test_directed(dut):
    ...
    for fop in (0, 1):
        await check(dut, fop, a, b, old, 8, 0xFF)      # full
        await check(dut, fop, a, b, old, 0, 0xFF)      # vl=0: no writes
        await check(dut, fop, a, b, old, 5, 0b00101)   # partial vl + sparse mask
        await check(dut, fop, a, b, old, 8, 0b10101010)
    dut._log.info("atum_vfpu: directed fp corners (zeros/inf/nan/subnormal) match golden")


@cocotb.test()
async def test_random(dut):
    rng = random.Random(0xF90A7)
    for _ in range(5000):
        fop = rng.randint(0, 1)
        ...
        await check(dut, fop, vs1, vs2, old, vl, mask)
    dut._log.info("atum_vfpu: 5000 random fp32 vector ops match golden (vfadd/vfmul, all vl/mask)")

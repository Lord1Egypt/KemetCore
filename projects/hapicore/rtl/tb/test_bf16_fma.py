"""cocotb testbench for hapi_bf16_fma — bit-exact vs single-rounded golden fp_fma."""
import cocotb
...
async def test_random(dut):
    await fc.run_random(dut, FMT, 25000, 0xB16FA0)


@cocotb.test()
async def test_cancellation(dut):
    await fc.run_cancellation(dut, FMT, 12000, 0xB16CA7)


@cocotb.test()
async def test_opposite_sign_tail(dut):
    await fc.run_opposite_sign_tail(dut, FMT, 16000, 0xB16077)

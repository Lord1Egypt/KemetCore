"""cocotb testbench for SethCore seth_pipeline_csr — single-cycle RV32IMZicsr core,
...
async def run_and_check(dut, words, cycles, tag, irq=(0, 0, 0)):
    ...
    # run dut for enough cycles to reach the self-loop and stabilize
    ...
    # run golden for enough cycles to reach the self-loop
    for _ in range(cycles * 3):
        ref.step()
    # compare regfile
    for i in range(1, 32):
    ...
    dut._log.info(f"seth_pipeline_csr [{tag}] matches CpuZ (regs + CSRs)")
